...
Vector2f bullet_spawn(Entity host, float angle)
{
	...
	float x_dist = ((host_rect.getSize().x) / 2.f) + (bullet_size.x / 2.f); //How far from the host distance the bullet need to spawn
	float y_dist = ((host_rect.getSize().y) / 2.f) + (bullet_size.y / 2.f);
